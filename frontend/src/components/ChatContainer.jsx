...
const ChatContainer = () => {
    ...
    const [isLoading, setIsLoading] = useState(false); // Moved from useChat
    const [messages, setMessages] = useState([]); // Moved from useChat
    ...
    const { isListening, startListening, stopListening, voiceEnabled, setVoiceEnabled, speak, isSpeaking } = useVoice((text) => {
        setInputText(text);
        handleSend(text);
    });
    ...
    useEffect(() => {
        scrollToBottom();
    }, [messages, thoughts]); // Added thoughts to dependency array



    const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:8000'; // Define API_URL here

    const handleFileUpload = async (e) => {
        const file = e.target.files[0];
        if (!file) return;

        // Handle PDFs (Existing Logic)
        if (file.type === 'application/pdf') {
            ...
            try {
                ...
                const data = await response.json(); // Assuming the backend might return some data
                setMessages(prev => [...prev, {
                    role: 'assistant',
                    content: `I've processed ${file.name}. You can now ask me questions about it! 📄`
                }]);
            } catch (error) {
    ...
    const handleSend = async (textOverride = null) => {
        ...
        setInputText('');
        // setTranscript(''); // Clear voice transcript - removed as transcript is handled differently now
        ...
        try {
            const response = await fetch(`${API_URL}/api/chat`, {
                ...
                body: JSON.stringify({
                    message: text,
                    history: messages, // Note: This sends the current 'messages' state, which might not include the very last user message yet. Consider updating history to include userMessage before sending.
                    model: currentModel
                }),
            });
    ...
    return (
        <div className="flex flex-col h-screen max-w-2xl mx-auto bg-white shadow-xl overflow-hidden md:rounded-xl md:h-[90vh] md:my-[5vh] relative">
            ...
            <ThoughtLogger thoughts={thoughts} isThinking={isLoading} />
            ...
            <div className="p-4 bg-white border-t border-gray-100">
                <div className="flex items-end gap-2">
                    ...
                    <button
                        ...
                        className={`p-3 rounded-full transition-colors ${isUploading ? 'bg-gray-100 text-gray-400 animate-pulse' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                        title="Upload PDF"
                    >
